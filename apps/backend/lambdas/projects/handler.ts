...
async function getDashboard({ event }: RouteCtx): Promise<APIGatewayProxyResult> {
...
async function getMembers({ params }: RouteCtx): Promise<APIGatewayProxyResult> {
  const id = params.id;
...
// GET /projects
async function listProjects(): Promise<APIGatewayProxyResult> {
  const projects = await db.selectFrom('branch.projects').selectAll().execute();
  return json(200, projects);
}

// GET /projects/{id}/donors
async function getDonors({ event, params }: RouteCtx): Promise<APIGatewayProxyResult> {
  const id = params.id;
  if (!id) return json(400, { message: 'id is required' });
  if (isNaN(Number(id))) {
...
async function getExpenditures({ params }: RouteCtx): Promise<APIGatewayProxyResult> {
  const id = params.id;
  if (!id) return json(400, { message: 'id is required' });
...
// GET /projects/{id}
async function getProject({ params }: RouteCtx): Promise<APIGatewayProxyResult> {
  const id = params.id;
  if (!id) return json(400, { message: 'id is required' });
...
// PUT /projects/{id}
async function updateProject({ event, params }: RouteCtx): Promise<APIGatewayProxyResult> {
  const id = params.id;
  if (!id) return json(400, { message: 'id is required' });
...
// POST /projects
async function createProject({ event }: RouteCtx): Promise<APIGatewayProxyResult> {
...
export const handler = (event: any): Promise<APIGatewayProxyResult> =>
  dispatch(event, {
    prefix: 'projects',
    routes: [
      // dashboard must precede `/projects/:id` (both 2-segment GETs)
      { method: 'GET', pattern: '/projects/dashboard', handler: getDashboard },
      { method: 'GET', pattern: '/projects/:id/members', handler: getMembers },
      { method: 'GET', pattern: '/projects/:id/donors', handler: getDonors },
      { method: 'GET', pattern: '/projects/:id/expenditures', handler: getExpenditures },
