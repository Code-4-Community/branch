...
export const handler = async (event: any): Promise<APIGatewayProxyResult> => {
  try {
    ...
    // Health check
    if (
      (normalizedPath.endsWith('/health') || normalizedPath === '/health') &&
      method === 'GET'
    ) {
    ...
    // POST /test/operation
    if (normalizedPath === '/test/operation' && method === 'POST') {
      const body = event.body
        ? (JSON.parse(event.body) as Record<string, unknown>)
        : {};
      // TODO: Add your business logic here
      return json(200, { ok: true, route: 'POST /test/operation', body });
    }
    // <<< ROUTES-END
