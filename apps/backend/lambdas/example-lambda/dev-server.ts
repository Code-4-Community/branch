...
const HANDLER_NAME = 'example-lambda';
...
async function startOrJoinServer() {
  ...
  try {
    const response = await fetch(
      `http://localhost:${SHARED_SERVER_PORT}/_register`,
      {
        method: 'POST',
    ...
    if (response.ok) {
      ...
      console.log(
        `Handler available at: http://localhost:${SHARED_SERVER_PORT}${BASE_PATH}`,
      );
      console.log(
        `Swagger UI: http://localhost:${SHARED_SERVER_PORT}${BASE_PATH}/swagger`,
      );
      return;
    }
  } catch (err) {
    ...
    try {
      ...
      req.on('end', async () => {
        ...
        const fullUrl = new URL(
          req.url || '/',
          `http://localhost:${SHARED_SERVER_PORT}`,
        );
        ...
        if (fullUrl.pathname === '/_register' && req.method === 'POST') {
          ...
          try {
            ...
            const swaggerUtils = require(path.join(
              handlerPath,
              'swagger-utils.ts',
            ));
...
<body>
  ...
  <ul>${handlerList}</ul>
        ...
        if (!handlers.has(handlerName)) {
          ...
          res.end(
            JSON.stringify({ message: `Handler '${handlerName}' not found` }),
          );
          return;
        }
        ...
        if (handlerPath === '/swagger' && req.method === 'GET') {
          const html = swaggerUtils.getSwaggerHtml(
            `/${handlerName}/swagger.json`,
          );
          res.statusCode = 200;
          res.setHeader('Content-Type', 'text/html');
          res.end(html);
  ...
  server.listen(SHARED_SERVER_PORT, () => {
    console.log(
      `Lambda Dev Server started on http://localhost:${SHARED_SERVER_PORT}`,
    );
    console.log(
      `Available handlers will be listed at http://localhost:${SHARED_SERVER_PORT}`,
    );
    ...
    console.log(
      `Handler '${HANDLER_NAME}' available at: http://localhost:${SHARED_SERVER_PORT}${BASE_PATH}`,
    );
    console.log(
      `Swagger UI: http://localhost:${SHARED_SERVER_PORT}${BASE_PATH}/swagger`,
    );
