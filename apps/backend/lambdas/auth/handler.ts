...
export const handler = async (event: any): Promise<APIGatewayProxyResult> => {
  try {
    ...
    // POST /mfa-setup
    if (normalizedPath === '/mfa-setup' && method === 'POST') {
      return await handleMfaSetup(event);
    }

    // POST /mfa-verify
    if (normalizedPath === '/mfa-verify' && method === 'POST') {
      return await handleMfaVerify(event);
    }

    // POST /mfa-disable
    if (normalizedPath === '/mfa-disable' && method === 'POST') {
      return await handleMfaDisable(event);
    }

    // GET /mfa-status
    if (normalizedPath === '/mfa-status' && method === 'GET') {
      return await handleMfaStatus(event);
    }
    // <<< ROUTES-END
 ...
 * with the caller's own AccessToken -- user-context Cognito calls, not Admin*
 * ones, so no IAM change was needed to grant them (see infrastructure/AGENTS.md).
 */
 ...
 * POST /mfa-verify using the *same* secret's current code, not a stale one from
 * an earlier call.
 */
async function handleMfaSetup(event: any): Promise<APIGatewayProxyResult> {
 ...
 */
async function handleMfaVerify(event: any): Promise<APIGatewayProxyResult> {
  const accessToken = getBearerAccessToken(event);
  if (!accessToken) {
    return json(401, { message: 'Authorization header is required' });
  }
 ...
 * /mfa-setup issues a new one, so a disabled-then-re-enabled account never
 * silently trusts the old code.
 */
async function handleMfaDisable(event: any): Promise<APIGatewayProxyResult> {
  const accessToken = getBearerAccessToken(event);
  if (!accessToken) {
    return json(401, { message: 'Authorization header is required' });
  }
...
/** GET /mfa-status -- whether the signed-in user currently has TOTP MFA enabled. */
async function handleMfaStatus(event: any): Promise<APIGatewayProxyResult> {
  const accessToken = getBearerAccessToken(event);
  if (!accessToken) {
    return json(401, { message: 'Authorization header is required' });
  }
