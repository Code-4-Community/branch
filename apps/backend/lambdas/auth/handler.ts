...
async function handleVerifyEmail({ event }: RouteCtx): Promise<APIGatewayProxyResult> {
...
async function handleResendCode({ event }: RouteCtx): Promise<APIGatewayProxyResult> {
  const body = event.body ? JSON.parse(event.body) as Record<string, unknown> : {};
  const { email } = body;
  if (!email) {
...
// POST /auth/logout
async function handleLogout({ event }: RouteCtx): Promise<APIGatewayProxyResult> {
...
async function handleForgotPassword({ event }: RouteCtx): Promise<APIGatewayProxyResult> {
  const body = event.body ? JSON.parse(event.body) as Record<string, unknown> : {};
  const { email } = body;
  if (!email) {
    return json(400, { message: 'email is required' });
...
async function handleResetPassword({ event }: RouteCtx): Promise<APIGatewayProxyResult> {
  const body = event.body ? JSON.parse(event.body) as Record<string, unknown> : {};
...
export const handler = (event: any): Promise<APIGatewayProxyResult> =>
  dispatch(event, {
    ...
    routes: [
      ...
      { method: 'POST', pattern: '/auth/resend-code', handler: handleResendCode },
