...
Output-only helpers consumed by the roots: `tags` (the `Project = branch` default tag) and `sentry` (layer ARN + DSN + `NODE_OPTIONS`, pinned in one place so `aws/` and `preview/` cannot drift). The DSN is committed deliberately — it only grants event-ingest write to a single project. Bumping the SDK is a version bump on the ARN here; the layer is region-pinned to `us-east-2`.
