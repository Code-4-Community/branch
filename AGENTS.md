...
- **Infra:** Terraform 1.13.0, state in S3 (`branch-tf-state`, S3-native locking), secrets via Infisical. Frontend is a static export on S3 + CloudFront (SPA); backend on Lambda + API Gateway; RDS Postgres 17.
